...
<script type="text/javascript">
   var banner_url,folder_name,numfiles,token;
   token = '<?php echo md5(DB_NAME); ?>';
       ...
       if(document.getElementById('test-list'))  // Sorting
       {
        banner("#test-list").sortable({
                ...
                update : function () {
                    ...
                    banner("#info").load(banner_url+"/wp-content/plugins/"+folder_name+"/admin/banner_ajax.php?"+order);
                   // location.reload();
                    }
                });
       } // Sorting Over
        ...
        //Multiple Upload Of Images
         banner('#swfupload-control').swfupload({
            ...
            .bind('uploadSuccess', function(event, file, serverData){
                ...
                item.addClass('success').find('p.status').html('Done!!!Image uploaded successfully. You can give name, description in Quick edit');
            ...
            .bind('uploadComplete', function(event, file){
                ...
                if(j == i)
                    {
                       document.getElementById('banner_uplimage').style.visibility = 'visible';
                       document.getElementById('banner_uplimage_two').style.visibility = 'visible';
                    }
                    i++
...
<style>
    ...
    <script type="text/javascript">
				function deleteImages(){
					if(document.getElementById('action_images').selectedIndex == 1)
					{
					...
					else if(document.getElementById('action_images').selectedIndex == 0)
					{
					return false;
					}
...
<div id="swfupload-control" class="upd_btn">
    ...
    <div id="banner_uplimage"><input type="button" value="Insert"  style="" onClick="window.location.reload()" /></div>
        <div class="listing lfloat">
           <input type="button" id="button" />
           <div class="lfloat prefer"><strong>Note:</strong> For the best performance, the uploaded image size (height & width) should be similar to the <a href="?page=banner_temp&style=<?php echo $publish_show->bann_tempid;?>" target="_blank">Banner style dimensions</a> specified in the banner style settings. 
           </div>
           <div class="clear"></div>
    ...
    <div id="banner_uplimage_two"><input type="button" value="Insert"  style="" onClick="window.location.reload()" /></div>
...
<?php } ?>
    <form name="banner_img" id="banner_img" method="POST" onsubmit="return deleteImages();">
        <div class="action_btn lfloat">
            <select id="action_images" name="action_images"  class="lfloat">
                    <option value="Bulk" selected="selected"><?php _e('Bulk Actions'); ?></option>
             ...
             <table cellspacing="0" cellpadding="0" border="1" width="50%" align="center" class="bann_file">
                    <thead class="bann_photos">
                        <tr class="height_tr">
                            ...
                            <th  class='img_table_name'>Banner Title</th>
                            <th  class='img_table_caption'>Banner Description</th>
...
if(count($result) > 0)
{
                    ...
                    foreach ($result as $results)
                    {
                     ...
                     $banner .="<td class='img_table_img'>
                                <a id='$banner_url/wp-content/uploads/apptha_banner_images/thumbnails/$results->bann_img' class='preview' alt='Edit' href='javascript:void(0)'>
                                <img src='$banner_url/wp-content/uploads/apptha_banner_images/thumbnails/$results->bann_img' width='50' height='50' /></a></td>";
...
{
    echo '<tr><td colspan="7" style="font-weight:bold">No images<tr></td>';
}
                    ?>
               </tbody>
             </table>
    ...
    <input type="hidden" name="token" id="token" value="<?php echo md5(DB_NAME);?>"/>